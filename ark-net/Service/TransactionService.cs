...
namespace ArkNet.Service
{
    public static class TransactionService
    {
        ...
        {
            var res = 0;
            var peerURLs = PeerService.GetAll().Peers.Where(x => x.Status.Equals("OK")).OrderByDescending(x => x.Height).Take(ArkNetApi.Instance.NetworkSettings.MaxNumOfBroadcasts).Select(x => new { Ip = x.Ip, Port = x.Port }).ToList();

            for (var i = 0; i < ArkNetApi.Instance.NetworkSettings.MaxNumOfBroadcasts; i++)
            {
                var response = PostTransaction(transaction, new PeerApi(peerURLs[i].Ip, peerURLs[i].Port));
        ...
        public async static Task<int> MultipleBroadCastAsync(TransactionApi transaction)
        {
            ...
            var peerURLs = peers.Peers.Where(x => x.Status.Equals("OK")).OrderByDescending(x => x.Height).Take(ArkNetApi.Instance.NetworkSettings.MaxNumOfBroadcasts).Select(x => new { Ip = x.Ip, Port = x.Port }).ToList();

            for (var i = 0; i < ArkNetApi.Instance.NetworkSettings.MaxNumOfBroadcasts; i++)
            {
                var response = await PostTransactionAsync(transaction, new PeerApi(peerURLs[i].Ip, peerURLs[i].Port));
